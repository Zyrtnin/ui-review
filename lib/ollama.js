 ...
 * Uses the /api/generate endpoint with an empty prompt and keep_alive
 * to load the model without generating tokens. This avoids the long
 * TTFT (time to first token) that can exceed CF tunnel idle timeouts.
 *
...
export async function prewarm(config) {
  return withRetry(async () => {
    ...
    const response = await ollamaFetch(url, {
      ...
      signal: AbortSignal.timeout(120_000), // 2 min — model load only, no inference
    }, config);
    ...
    // Drain the response body (should be minimal for empty prompt)
    try {
      for await (const _ of response.body) { /* drain */ }
    } catch {
      // Ignore — model is loaded regardless
    }
  }, config);
}
