...
/**
 * Execute a sequence of pre-screenshot actions on a page.
 * Actions are declarative objects: { click: sel }, { fill: sel, value: text }, etc.
 */
async function executeActions(page, actions) {
  ...
  for (const action of actions) {
    if (action.click) {
      await page.click(action.click, { timeout: ACTION_TIMEOUT });
    } else if (action.fill !== undefined) {
      await page.fill(action.fill, action.value || '', { timeout: ACTION_TIMEOUT });
    } else if (action.hover) {
      await page.hover(action.hover, { timeout: ACTION_TIMEOUT });
    } else if (action.wait) {
      await page.waitForSelector(action.wait, { timeout: ACTION_TIMEOUT });
    } else if (action.select !== undefined) {
      await page.selectOption(action.select, action.value || '', { timeout: ACTION_TIMEOUT });
      ...
      await new Promise(r => setTimeout(r, Math.min(Math.max(0, action.delay), 10000)));
    }
  }
}
...
export async function captureScreenshot({
  ...
  if (existingPage) {
    try {
      ...
      await executeActions(existingPage, actions);
