...
function validateManifest(manifest) {
  ...
  if (manifest.pages) {
    ...
    for (const page of manifest.pages) {
      ...
      // Validate optional tokenAuth config
      if (page.tokenAuth) {
        const ta = page.tokenAuth;
        if (!ta.endpoint || typeof ta.endpoint !== 'string' || !ta.endpoint.startsWith('/')) {
          throw new ConfigError(`Page "${page.name}" tokenAuth.endpoint must be a string starting with /`);
        }
        if (ta.method && !['GET', 'POST'].includes(ta.method)) {
          throw new ConfigError(`Page "${page.name}" tokenAuth.method must be GET or POST`);
        }
        if (ta.body && typeof ta.body !== 'object') {
          throw new ConfigError(`Page "${page.name}" tokenAuth.body must be an object`);
        }
        if (!ta.responseField || typeof ta.responseField !== 'string') {
          throw new ConfigError(`Page "${page.name}" tokenAuth.responseField is required`);
        }
        if (!ta.queryParam || typeof ta.queryParam !== 'string') {
          throw new ConfigError(`Page "${page.name}" tokenAuth.queryParam is required`);
        }
      }

      // Validate optional actions array
      if (page.actions) {
        if (!Array.isArray(page.actions)) {
          throw new ConfigError(`Page "${page.name}" actions must be an array`);
        }
        ...
        for (let i = 0; i < page.actions.length; i++) {
          ...
          if (typeof action !== 'object' || action === null) {
            throw new ConfigError(`Page "${page.name}" actions[${i}] must be an object`);
          }
          const actionType = Object.keys(action).find(k => validKeys.has(k));
          if (!actionType) {
            throw new ConfigError(`Page "${page.name}" actions[${i}] has no recognized type. Valid: ${[...validKeys].join(', ')}`);
          }
          if (action.delay !== undefined && (typeof action.delay !== 'number' || action.delay < 0 || action.delay > 10000)) {
            throw new ConfigError(`Page "${page.name}" actions[${i}].delay must be 0-10000 ms`);
          }
          if ((action.fill !== undefined || action.select !== undefined) && typeof action.value !== 'string') {
            throw new ConfigError(`Page "${page.name}" actions[${i}] requires a "value" string`);
          }
        }
      }
    }
